...
object AWSEBPlugin extends sbt.AutoPlugin {
  ...
  override lazy val buildSettings =
    Seq(
      ...
      listAvailableSolutionStacks in awseb := listAvailableSolutionStacksTask.value
    )
  ...
  override lazy val projectSettings =
    Seq(
      ...
      s3AppBucketName in awseb := "sbt-awseb-bundle-" + (ebAppName in awseb).value,
      ebAppVersionLabel in awseb := s"${version.value}-${ISO8601.timestamp()}",
      ebEnvMap in awseb := Map.empty[String, EBEnvironment],
      checkDNSAvailability in awseb := checkDNSAvailabilityTask,
      cleanApplicationVersions in awseb := cleanApplicationVersionsTask.value,
      createAppBucket in awseb := createAppBucketTask.value,
      createApplication in awseb := createApplicationTask.value,
      createApplicationVersion in awseb := createApplicationVersionTask.evaluated,
      createEnvironment in awseb := createEnvironmentTask.evaluated,
      deleteAppBucket in awseb := deleteAppBucketTask.value,
      deleteApplication in awseb := deleteApplicationTask.value,
      deleteApplicationVersion in awseb := deleteApplicationVersionTask,
      describeApplication in awseb := describeApplicationTask.value,
      describeApplicationVersions in awseb := describeApplicationVersionsTask.value,
      ...
      describeEnvironments in awseb := describeEnvironmentsTask.value,
      ...
      updateApplication in awseb := updateApplicationTask.value,
      updateApplicationVersion in awseb := updateApplicationVersionTask.inputTaskValue,
      updateEnvironmentVersion in awseb := updateEnvironmentVersionTask.evaluated,
      uploadAppBundle in awseb := uploadAppBundleTask.value
    )
}
